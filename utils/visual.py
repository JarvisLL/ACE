...
def get_color_pallete(npimg, dataset='pascal_voc'):
    ...
    if dataset == 'ade20k':
        ...
        out_img = Image.fromarray(npimg.astype('uint8'))
        out_img.putpalette(mapillarypallete)
        return out_img
    out_img = Image.fromarray(npimg.astype('uint8'))
...
mapillarypallete = [
    220, 20,  60,   #0 person
    119, 11,  32,   #1 vehicule
    250, 170, 30,   #2 traffic light
    220, 220, 0,    #3 traffic sign front
    255, 255, 255,  #4 lane marking
    128, 64,  128,  #5 road
    196, 196, 196,  #6 curb
    96,  96,  96,   #7 pedestrain area
    140, 140, 200,  #8 crosswalk
    230, 150, 140,  #9 dangerous area
    190, 153, 153,  #10 fence
    100, 128, 160,  #11 road construct
    153, 153, 153,  #12 pole
    16,  207, 248,  #13 traffic sign back
    140, 140, 140,  #14 trash can
    0,   0,   255,  #15 bicycle
    0,   0,   0,    #16 unlabel
]
